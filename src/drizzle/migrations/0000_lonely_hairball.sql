CREATE TABLE `app_priority` (
	...
	CONSTRAINT `app_priority_id` PRIMARY KEY(`id`),
	CONSTRAINT `app_priority_name_unique` UNIQUE(`name`)
);
...
--> statement-breakpoint
ALTER TABLE `app_screenshots` ADD CONSTRAINT `app_screenshots_app_id_apps_id_fk` FOREIGN KEY (`app_id`) REFERENCES `apps`(`id`) ON DELETE no action ON UPDATE no action;--> statement-breakpoint
...
ALTER TABLE `version_logs` ADD CONSTRAINT `version_logs_version_id_versions_id_fk` FOREIGN KEY (`version_id`) REFERENCES `versions`(`id`) ON DELETE no action ON UPDATE no action;--> statement-breakpoint
ALTER TABLE `versions` ADD CONSTRAINT `versions_app_id_apps_id_fk` FOREIGN KEY (`app_id`) REFERENCES `apps`(`id`) ON DELETE no action ON UPDATE no action;