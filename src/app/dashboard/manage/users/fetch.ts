...
import { FetchUsersData } from "@/app/api/internal/user/route";
import {
    getBaseUrl,
    getSessionCookie,
} from "@/lib/server_utils";
import { GetUsers_C_Tag } from "@/repositories/users";
import { FetchDeleteUser } from "@/app/api/internal/user/[user_id]/delete/route";
import { z } from "zod";
import {
    createUserFormSchema,
} from "@/app/api/internal/user/schema";
import { FetchCreateUser } from "@/app/api/internal/user/create/route";
...
export async function fetchUsers(): ResponseJson<FetchUsersData> {
    try {
        ...
        const response = await fetch(
            `${await getBaseUrl()}/api/internal/user`,
            {
                method: "GET",
                headers: {
...
export async function fetchCreateUser(
    ...
    try {
        const sessionId = await getSessionCookie();
        const response = await fetch(
            `${await getBaseUrl()}/api/internal/user/create`,
            {
                method: "POST",
                headers: {
                    Authorization: `Bearer ${sessionId}`,
...
export async function fetchDeleteUserById(
    ...
    try {
        const sessionId = await getSessionCookie();
        const response = await fetch(
            `${await getBaseUrl()}/api/internal/user/${userId}/delete`,
            {
                method: "DELETE",
                headers: {
                    Authorization: `Bearer ${sessionId}`,
