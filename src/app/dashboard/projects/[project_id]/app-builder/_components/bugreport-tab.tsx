          ...
          <div className="border border-[#E6E8EC] rounded-lg bg-white">
            {bugReports
              ...
              .map((report, idx, arr) => (
                <div
                  key={report.id}
                  className={idx !== arr.length - 1 ? 'relative after:content-[" "] after:absolute after:left-0 after:right-0 after:bottom-0 after:h-px after:bg-[#E6E8EC]' : ''}
                  style={{ fontFamily: 'Inter', padding: '24px', border: 'none', borderRadius: 0, marginBottom: 0, background: 'transparent' }}
                >
                    ...
                    <div style={{ display: 'flex', flexDirection: 'row', alignItems: 'center', gap: '12px' }}>
                      ...
                      <div className="w-8 h-8 rounded-full bg-[#B1B5C3] flex items-center justify-center font-bold" style={{ color: 'var(--OnBackground, #000)', fontFamily: 'Inter', fontSize: '14px', fontStyle: 'normal', fontWeight: 400, lineHeight: '20px' }}>
                        {report.tester && report.tester.firstName
                          ? report.tester.firstName.charAt(0).toLowerCase()
                  ...
                  <div className="flex flex-row gap-2 mt-5">
                    {report.image && (
                      <img
                        ...
                        style={{ width: '50%', minWidth: 0, maxWidth: '50%', height: 'auto' }}
                        onClick={() => setFullscreenImage(report.image!)}
                        onError={e => { e.currentTarget.onerror = null; e.currentTarget.src = '/placeholders/placeholder.png'; }}
                      />
                    )}
                    {report.video && (
                      <video
                        ...
                        className="object-cover cursor-pointer aspect-[16/9] rounded"
                        style={{ width: '50%', minWidth: 0, maxWidth: '50%', height: 'auto' }}
                        onClick={() => setFullscreenVideo(report.video!)}
                      />
                    )}
                  </div>
                )}
