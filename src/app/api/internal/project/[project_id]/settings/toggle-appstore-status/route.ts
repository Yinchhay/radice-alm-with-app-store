...
import { updateProjectAppStatusSchema } from "../../schema";
...
export async function PATCH(request: Request, { params }: Params) {
    try {
        ...
        let body: z.infer<typeof updateProjectAppStatusSchema> =
            await request.json();
        const validationResult =
            updateProjectAppStatusSchema.safeParse(body);
        ...
        await updateIsAppStatus(Number(params.project_id), body.status)
        // await updateProjectPublicApp(Number(params.project_id), body.status);
