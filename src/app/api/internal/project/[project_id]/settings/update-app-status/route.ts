...
const successMessage = "Successfully updated project settings appStatus";
const unsuccessMessage = "Failed to update project settings appStatus";

type Params = { params: { project_id: string } };
export type FetchUpdateProjectSettingsAppStatus = Record<string, unknown>;
...
export async function PATCH(request: Request, { params }: Params) {
    try {
        ...
        await updateProjectAppStatus(Number(params.project_id), body.status);

        return buildSuccessResponse<FetchUpdateProjectSettingsAppStatus>(
            successMessage,
            {},
        );
    } catch (error: any) {
